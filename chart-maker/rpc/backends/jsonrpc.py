...
class JSONRPCDaemon(object):
    ...
    def __init__(self, protocol='http', host='0.0.0.0', port=11181, path='/json_rpc',
    ...
    def block(self, bhash=None, height=None):
        ...
        if res['status'] != 'OK' or res2['status'] != 'OK':
            raise exceptions.BackendException(res['status'])
        ...
        res = self.raw_request('/get_transactions', {
                'txs_hashes': sub_json['tx_hashes'],
                'decode_as_json': True})
        if res['status'] != 'OK':
            raise exceptions.BackendException(res['status'])
        txs = []
        for tx in res.get('txs', []):
            txs.append(json.loads(tx['as_json']))
        data['locked_txs'] = [(sum(y['amount'] for y in x['vout'])/(10**12),x['unlock_time']) for x in txs if x['unlock_time']>0]
        data['inputs'] = sum([len(x['vin']) for x in txs])
        data['outputs'] = sum([len(x['vout']) for x in txs])
        data['volume'] = sum([sum([y['amount'] for y in x['vout']]) for x in txs])
        return Block(**data)
