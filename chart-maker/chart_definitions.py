definitions = {
    'block_count_24h': {
        'title' : 'Block Count (Previous 24h)',
        'file-name' : 'block_count_24h',
        'x-axis' : 'date',
        'y-axis' : 'Blocks',
        'lineWidth' : '1',
        'logScale' : 'true',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'block_difficulty': {
        'title' : 'Network Difficulty',
        'file-name' : 'block_difficulty',
        'x-axis' : 'Date',
        'y-axis' : 'Hashes/Block',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'block_height': {
        'title' : 'Block Height',
        'file-name' : 'block_height',
        'x-axis' : 'Date',
        'y-axis' : 'Blocks',
        'logScale' : 'none',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'block_reward': {
        'title' : "Miner's Block Reward (Per Block)",
        'file-name' : 'block_reward',
        'x-axis' : 'Date',
        'y-axis' : 'Coins',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'block_size_cum': {
        'title' : 'Blockchain Size',
        'file-name' : 'block_size_cum',
        'x-axis' : 'Date',
        'y-axis' : 'b',
        'logScale' : 'none',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'block_size_30d': {
        'title' : 'Block Size Sum 30 days',
        'file-name' : 'block_size_30d',
        'x-axis' : 'Date',
        'y-axis' : 'b',
        'logScale' : 'none',
        'curveType' : 'none',
        'lineWidth' : '1',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'fee_24h': {
        'title' : 'Fee 24h',
        'file-name' : 'fee_24h',
        'x-axis' : 'Date',
        'y-axis' : 'Coins',
        'lineWidth' : '1',
        'logScale' : 'true',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'fee_24h_usd': {
        'title' : 'Fee 24h $',
        'file-name' : 'fee_24h_usd',
        'x-axis' : 'Date',
        'y-axis' : '$',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'inflation_1Y': {
        'title' : 'Inflation Rate',
        'file-name' : 'inflation_1Y',
        'x-axis' : 'Date',
        'y-axis' : '% Coins Created in Past Year',
        'logScale' : 'none',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '###.000',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } ,
    'inputs': {
        'title' : 'Tx Inputs 24h',
        'file-name' : 'inputs',
        'x-axis' : 'Date',
        'y-axis' : '#',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '######',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'marketcap': {
        'title' : 'Value of Circulating Supply',
        'file-name' : 'marketcap',
        'x-axis' : 'Date',
        'y-axis' : '$',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'marketcap_btc': {
        'title' : 'Value of Circulating Supply (BTC)',
        'file-name' : 'marketcap_btc',
        'x-axis' : 'Date',
        'y-axis' : 'BTC',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'marketcap_xmr': {
        'title' : 'Value of Circulating Supply (XMR)',
        'file-name' : 'marketcap_xmr',
        'x-axis' : 'Date',
        'y-axis' : 'XMR',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'miner_revenue_1Y': {
        'title' : "1Y Miner's Revenue (365 Days' Emission + Fees)",
        'file-name' : 'miner_revenue_1Y',
        'x-axis' : 'Date',
        'y-axis' : '$',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'miner_revenue_30d': {
        'title' : "30d Miner's Revenue (Emission + Fees)",
        'file-name' : 'miner_revenue_30d',
        'x-axis' : 'Date',
        'y-axis' : '$',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } ,
    'nonce': {
        'title' : 'Nonces',
        'file-name' : 'nonce',
        'x-axis' : 'Date',
        'y-axis' : 'Nonce Value Normalized by Maximum Nonce Value',
        'logScale' : 'none',
        'lineWidth' : '0',
        'pointSize' : '0.05',
        'curveType' : 'none',
        'format' : '##.###',
        'type': 'number',
        'chartType': 'ScatterChart',
        'interval' : '50',
    } , 
    'nonce_dist': {
        'title' : 'Nonce Distribution as Compared to a Uniform Distribution',
        'file-name' : 'nonce_dist',
        'x-axis' : 'Date',
        'y-axis' : 'Uniformity',
        'lineWidth' : '1',
        'logScale' : 'none',
        'curveType' : 'function',
        'format' : '##.###',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'outputs': {
        'title' : 'Tx Outputs 24h',
        'file-name' : 'outputs',
        'x-axis' : 'Date',
        'y-axis' : '#',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '######',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'outputs_inputs': {
        'title' : 'Tx Outputs minus Inputs cumulative',
        'file-name' : 'outputs_inputs',
        'x-axis' : 'Date',
        'y-axis' : '#',
        'logScale' : 'none',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '######',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'price': {
        'title' : 'Price',
        'file-name' : 'price',
        'x-axis' : 'Date',
        'y-axis' : '$',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'price_btc': {
        'title' : 'Price (BTC)',
        'file-name' : 'price_btc',
        'x-axis' : 'Date',
        'y-axis' : 'BTC',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'price_xmr': {
        'title' : 'Price (XMR)',
        'file-name' : 'price_xmr',
        'x-axis' : 'Date',
        'y-axis' : 'XMR',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'supply_circulating': {
        'title' : 'Circulating Supply',
        'file-name' : 'supply_circulating',
        'x-axis' : 'Date',
        'y-axis' : 'Coins',
        'logScale' : 'none',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'supply_locked': {
        'title' : 'Locked Aeon',
        'file-name' : 'supply_locked',
        'x-axis' : 'Date',
        'y-axis' : 'Coins',
        'logScale' : 'none',
        'lineWidth' : '1',
        'curveType' : 'none',
        'format' : '###.000',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } ,
    'supply_total': {
        'title' : 'Total Supply',
        'file-name' : 'supply_total',
        'x-axis' : 'Date',
        'y-axis' : 'Coins',
        'logScale' : 'none',
        'lineWidth' : '1',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'tx_24h': {
        'title' : 'Transactions (Previous 24h)',
        'file-name' : 'tx_24h',
        'x-axis' : 'Date',
        'y-axis' : 'Transactions',
        'lineWidth' : '1',
        'logScale' : 'true',
        'curveType' : 'function',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'version': {
        'title' : 'Version',
        'file-name' : 'version',
        'x-axis' : 'Date',
        'y-axis' : '#',
        'logScale' : 'none',
        'curveType' : 'none',
        'lineWidth' : '1',
        'format' : '##.#',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'volume_30d': {
        'title' : 'Transaction Output Volume 30d',
        'file-name' : 'volume_30d',
        'x-axis' : 'Date',
        'y-axis' : 'Coins',
        'logScale' : 'none',
        'lineWidth' : '1',
        'curveType' : 'none',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'volume_usd_30d': {
        'title' : 'Transaction Output Volume $ 30d',
        'file-name' : 'volume_usd_30d',
        'x-axis' : 'Date',
        'y-axis' : '$',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'none',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
    'volume_btc_30d': {
        'title' : 'Transaction Output Volume BTC 30d',
        'file-name' : 'volume_btc_30d',
        'x-axis' : 'Date',
        'y-axis' : 'BTC',
        'logScale' : 'true',
        'lineWidth' : '1',
        'curveType' : 'none',
        'format' : '0.000E+00',
        'type': 'number',
        'chartType': 'LineChart',
        'pointSize' : '0',
        'interval' : '100',
    } , 
}
















